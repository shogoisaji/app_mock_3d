...
struct SettingsView: View {
    ...
    var body: some View {
        ...
        BottomSheetManager(isOpen: $showingAspectRatioSettings, height: 350) {
            AspectRatioSettingsView(settings: $settings, isPresented: $showingAspectRatioSettings)
        }
        
        // Background Bottom Sheet
        BottomSheetManager(isOpen: $showingBackgroundSettings, height: 400) {
            BackgroundSettingsView(settings: $settings, isPresented: $showingBackgroundSettings)
        }
        
        // Device Selection Bottom Sheet
        BottomSheetManager(isOpen: $showingDeviceSelection, height: 300) {
            DeviceSelectionView(settings: $settings, isPresented: $showingDeviceSelection)
        }
    }
